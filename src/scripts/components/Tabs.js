class Tabs {
	constructor($element) {
		...
		this.$navItems = $element.querySelectorAll('.tabs__nav-item')
		this.$accordionHandlers = $element.querySelectorAll('.tabs__item-header')
	...
	countOpenTabs() {
		return [...this.$tabs]
			.filter($tab => $tab.classList.contains('is-open'))
			.length
	}
}
